...
def eval_policy(params, config):
    sys.path.append(os.path.join(os.path.dirname(__file__), 'envs')) 
    env = build_environment(config)
    torch.manual_seed(config.seed)
    np.random.seed(config.seed)

    policy = GaussianPolicy(env.observation_space.shape[0], env.action_space.shape[0], config.hidden_size, env.action_space).to(config.device)
